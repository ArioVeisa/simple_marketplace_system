<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-dark-100">{{ __('Create User') }}</h2>
    </x-slot>
                    ...
                    <div class="grid grid-cols-2 gap-3 mt-2">
                        @foreach($roles as $role)
                            <label class="flex items-center gap-3 p-3 rounded-xl bg-dark-700/50 border border-dark-600 cursor-pointer hover:border-accent/30 transition-colors">
                                <input type="checkbox" name="roles[]" value="{{ $role->id }}" class="w-4 h-4 rounded border-dark-600 bg-dark-700 text-accent focus:ring-accent/30 focus:ring-offset-0">
