<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-dark-100">{{ __('Edit Product') }}</h2>
    </x-slot>
        ...
        <div class="glass-card p-6">
            <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" class="input-dark w-full px-4 py-3">{{ old('description', $product->description) }}</textarea>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <x-input-label for="price" :value="__('Price')" />
                        <x-text-input id="price" name="price" type="number" step="0.01" :value="old('price', $product->price)" required />
                    </div>
                    <div>
                        <x-input-label for="stock" :value="__('Stock')" />
                        <x-text-input id="stock" name="stock" type="number" :value="old('stock', $product->stock)" required />
                    </div>
                </div>
